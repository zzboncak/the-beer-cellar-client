...
class MainPage extends React.Component {
		...
		if (TokenService.hasAuthToken()) {
			fetch(`${config.API_ENDPOINT}/cellar`, {
				...
				.then((res) => {
					if (!res.ok) {
						throw new Error(
							"Could not load your beers! Sorry man..."
						);
					}
					return res.json();
				})
	...
	handleBeerDelete = (inventory_id) => {
		let newState = this.context.beers.filter(
			(beer) => beer.inventory_id !== inventory_id
		);
	...
	getAverageCellarRating = () => {
		let arrayOfAverages = this.context.beers.map(
			(beer) => beer.untappd_rating * beer.quantity
		);
	...
	getHighestRatedBeer = () => {
		...
		let sortedArray = arrayCopy.sort(
			(a, b) => a.untappd_rating - b.untappd_rating
		);
	...
	handleSort = (e) => {
		...
		if (sortOption === "quantity-low" || sortOption === "rating-low") {
			this.context.updateBeersInState(
				this.context.beers.sort((a, b) => a[keyName] - b[keyName])
			);
		} else if (
			sortOption === "quantity-high" ||
			sortOption === "rating-high"
		) {
			this.context.updateBeersInState(
				this.context.beers.sort((a, b) => b[keyName] - a[keyName])
			);
		} else if (sortOption === "a-z") {
			//sorting alpabetically
			this.context.updateBeersInState(
				this.context.beers.sort((a, b) => {
					let aName = a[keyName];
					let bName = b[keyName];
					if (aName < bName) {
						return -1;
		...
		return (
			<div className="user-login-page">
				...
				<section className="cellar">
					...
					<section className="beers-container">{beers}</section>
