...
class BeerResult extends React.Component {
	...
	handleAdd = (e) => {
		...
		if (TokenService.hasAuthToken()) {
			...
			let inventory_id =
				this.context.beers[this.context.beers.length - 1].inventory_id +
				1; //the inventory_id is required to delete the item later on for the user
			let newBeers = [
				...this.context.beers,
				{
		...
		return (
			<div className="beer-result">
				...
				<p className="beer-description">
					{this.props.beer_description}
				</p>
				<button id="add-cellar" onClick={this.handleAdd}>
					Add to my cellar
				</button>
			</div>
		);
